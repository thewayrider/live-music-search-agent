...
async function runAcidStagAgent(config = {}, exclusions = {}) {
  ...
  console.log("[AcidStag Agent] Feed items parsed:", feedItems.length);
  ...
  for (const item of feedItems) {
    ...
    const html = await fetchText(item.url, http, options).catch((err) => {
      console.warn("[AcidStag Agent] Failed to fetch roundup:", item.url, err.message);
      return null;
    });
    if (!html) continue;
  ...
  console.log("[AcidStag Agent] Total extracted before dedupe:", releases.length);
  
  // NOTE: Bypassing legacy MusicBrainz enrichment to get it running instantly
  const enriched = releases; 
  
  const finalReleases = dedupeAndSort(enriched, exclusions);

  // MAPPING TO UNIFIED AGENT FORMAT
  const unifiedResults = finalReleases.map(r => {
      return {
          ...
          description: `Score: ${r.score} | Genres: ${r.genresMapped.join(', ') || 'N/A'} | Type: ${r.releaseType}`
      };
  });
