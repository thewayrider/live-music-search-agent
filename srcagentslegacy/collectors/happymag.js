...
export async function collectHappyMagReleases(filters = {}, userOptions = {}) {
  ...
  console.log("Feed cards found:", feedCards.length);
  console.log("Feed card sample:", feedCards.slice(0, 5).map((card) => card.title));

  const targetCards = feedCards
    .filter((card) => isRelevantFeedCard(card, options))
    .slice(0, options.maxRoundupArticles);

  console.log("Target cards found:", targetCards.length);
  console.log("Target card sample:", targetCards.slice(0, 5).map((card) => card.title));
  ...
  for (const card of targetCards) {
    ...
    console.log("Article URL:", card.url);
    console.log("Article date:", article.releaseDate);
    console.log("Sections found:", article.sections.length);
    console.log(
      "Section sample:",
      article.sections.slice(0, 5).map((section) => section.heading)
    );

    const extracted = extractReleaseItems(article, card, filters, options);
    console.log("Extracted items:", extracted.length);

    releases.push(...extracted);
  }

  console.log("Total extracted before dedupe:", releases.length);
...
function extractReleaseItems(article, card, filters, options) {
  ...
  for (const section of article.sections) {
    ...
    items.push({
      ...
      releaseType,
      releaseDate: article.releaseDate || "unknown",  // ← restored (was missing)
...
function scoreRelease({ card, combinedText, releaseType, genresMapped, artistCountryMapped }) {
  ...
  // Country — primary signal for AU-focused pipeline
  if (artistCountryMapped === "Australia") score += 3;

  // Genre match — distinguish direct vs fallback
...
function dedupeAndSort(items, filters) {
  ...
  for (const item of items) {
    // Gate 1: article publish date — drop items from old articles entirely
    ...
    // Gate 2: individual release date — keep unknowns, drop known-old dates
